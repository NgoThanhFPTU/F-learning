...
const Timetablee = () => {
  ...
  const fetchTimetable = async (batchId) => {
    try {
      const timetableResponse = await axios.get('http://localhost:5000/timetable', { params: { batchId } });
  ...
  const handleSave = async (newTimetable) => {
    try {
      await axios.post('http://localhost:5000/timetable', { ...newTimetable, batchId: selectedBatch });
      fetchTimetable(selectedBatch);
    } catch (error) {
      console.error(error);
    }
